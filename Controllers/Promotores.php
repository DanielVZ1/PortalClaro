    ...
    class Promotores extends Controller{
      public function __construct(){
        ...
        public function registrar()
        {
          ...
          $fecha = date("YmdHis");
          if (empty($codigo) || empty($dni) || empty($nombre) || empty($apellido) || empty($telefono)
          || empty($profesion) || empty($direccion)) {
            $msg = "Todos los campos son obligatorios";
              ...
              if($id == ""){
                  $data =  $this->model->registrarPromotor($codigo, $dni, $nombre, $apellido, $telefono, $profesion,
                           $estado_civil, $genero, $direccion, $zona, $departamento, $municipio, $gerencia,$canal, $proyecto, $cargo, $imgNombre);
                  if ($data == "ok") {
                    if (!empty($name)){
                ...
                //if($_POST['foto_actual'] != $_POST['foto_delete']){
                  ...
                  $data=  $this->model->modificarPromotor($codigo, $dni, $nombre, $apellido, $telefono, $profesion,
                          $estado_civil, $genero, $direccion, $zona, $departamento, $municipio, $gerencia,$canal, $proyecto, $cargo, $imgNombre, $id);
                  if ($data == "modificado") {
                    if (!empty($name)){
                      move_uploaded_file($tmpname,$destino); //Para la foto
                    }
