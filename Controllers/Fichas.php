    ...
    class Fichas extends Controller{
      public function __construct(){
        ...
        public function registrar()
        {
          ...
          $foto = $_POST['foto'];
          $id = $_POST['id'];
          if (empty($codigoMaestro) || empty($nombre) || empty($apellido) || empty($dni) || empty($telefono) || empty($direccion) || 
            empty($zona) || empty($departamento) || empty($municipio) || empty($gerencia) || empty($canal) ||
            empty($proyecto) || empty($cargo) || empty($estadoCivil) || empty($genero) || empty($profesion) || empty($foto)) {
            $msg = "Todos los campos son obligatorios";
          }else{
              if($id == ""){
                  $data=  $this->model->registrarPromotor($codigoMaestro,$nombre,$apellido,$dni,$telefono,$direccion,$zona,
                          $departamento,$municipio,$gerencia,$canal,$proyecto,$cargo,$estadoCivil,$genero,$profesion,$foto);
                  if ($data == "ok") {
                    $msg = "si";
                  }else if ($data =="existe"){
                ...
                $data=  $this->model->modificarPromotor($codigoMaestro,$nombre,$apellido,$dni,$telefono,$direccion,$zona,
                $departamento,$municipio,$gerencia,$canal,$proyecto,$cargo,$estadoCivil,$genero,$profesion,$foto,$id);
                  if ($data == "modificado") {
                    $msg = "modificado";
                  }else{
